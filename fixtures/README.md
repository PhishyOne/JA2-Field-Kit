# Fixtures

This directory stores fixture metadata, not a public dump of JA2 saves.

- `provenance-manifest.json` is the public fixture registry.
- `provenance-manifest.schema.json` defines its machine-readable contract.
- `public/` is reserved for specifically reviewed, manifest-listed artifacts.
- `private/` is ignored and is the only staging location for locally supplied
  real saves and unapproved derived artifacts.

Read [`docs/fixture-policy.md`](../docs/fixture-policy.md) before adding or using
a fixture.

The supported validator runtime is CPython 3.12 on Linux x86-64. Create an
isolated environment and install the complete pinned closure before validation:

```bash
python3.12 -m venv .venv
.venv/bin/python -m pip install --require-hashes -r requirements/fixture-validation.lock
.venv/bin/python -m unittest discover -s tools/tests -v
head_oid=$(git rev-parse HEAD)
.venv/bin/python tools/validate_fixture_manifest.py \
  --subject "local-head=$head_oid"
```

`--subject` requires an explicit role and full commit object ID. Local runs use
one `local-head`; CI uses one `push-head`, or independent `pr-head` and
`integration-head` subjects bound to an exact base. Each requested commit and
every commit reachable through all of its parents is inspected directly from
Git objects. Generated bytes are measured independently from each subject's
read-only exact snapshot. Bubblewrap is required for generated fixtures; a
missing or nonfunctional sandbox fails closed. Generator execution uses a new
network namespace plus a seccomp backstop that denies socket and io_uring
syscalls. Hosted Ubuntu 24.04 CI performs an unprivileged capability smoke and,
only when stock AppArmor mediation blocks it without any competing bwrap policy,
loads Ubuntu's verified packaged `bwrap-userns-restrict` profile with add-only
semantics. It does not disable the host-wide user-namespace restriction.

On a trusted machine, require all cataloged local files to be present and match
their recorded size/digest with:

```bash
head_oid=$(git rev-parse HEAD)
.venv/bin/python tools/validate_fixture_manifest.py \
  --subject "local-head=$head_oid" \
  --check-local-files
```

The known JA2 Reborn Build `04.12.02` entry is intentionally cataloged with a
pending identity. Its source save is not in the repository, and it cannot be
used by a test until its real size and SHA-256 are recorded and verified.

The public synthetic header vector is generated only inside the validator's
exact-snapshot sandbox. Core test-resource preparation uses the validator's
narrow public materialization interface; it must never invoke the manifested
generator as a standalone subprocess.
