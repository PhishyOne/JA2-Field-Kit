...
| Generated-save editing/export (deferred) | Would access original bytes, requested edits, generated bytes, integrity facts, and output destination | Not implemented | Not implemented | Future explicit edit and export actions | Undecided; must not turn the input grant into implicit in-place write access | **Issue [#12](https://github.com/PhishyOne/JA2-Field-Kit/issues/12) is a mandatory design gate.** Require core-owned source-size admission before copying or proportional work, strictest-bound transaction-private candidate serialization, re-admission/reparse, verification of requested changes and critical unchanged facts, provenance, and fail-closed support before separate create-without-overwrite placement and placement verification; never overwrite the source by default. Replacement requires a continuous qualified guard from final expected-original comparison through atomic commit, post-commit verification, and outcome recording, plus independent backup and guarded recovery; otherwise it is unsupported and fails closed. See [the output-placement contract](transactional-edit-safety.md#output-placement-and-later-replacement) |
